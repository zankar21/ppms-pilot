...
export default function Logbook() {
  ...
  const [form, setForm] = useState({ date: "", shift: "A", unit: "UNIT-1", department: "MECH", summary: "" });
  ...
  const load = async () => {
    try { const r = await api.get("/api/logbook", { params: { unit, dept } }); setRows(r.data.rows || []); }
    catch (e) { alert(e?.response?.data?.error || e.message || "Failed to load logbook"); }
  };
  useEffect(() => { load(); /* eslint-disable-next-line */ }, [unit, dept]);

  const submit = async () => {
    if (!form.date || !form.unit) return;
    const payload = { date: form.date, shift: form.shift, unit: form.unit, department: form.department, summary: form.summary || "" };
    try { await api.post("/api/logbook", payload); setForm({ date: "", shift: "A", unit: "UNIT-1", department: "MECH", summary: "" }); load(); }
    catch (e) { alert(e?.response?.data?.error || e.message || "Failed to add logbook entry"); }
  };

  const show = (row) => {
    setEdit({ ...row, _paramsText: JSON.stringify(row.params || {}, null, 2), load_mw: row.load_mw ?? "", ambient_temp_c: row.ambient_temp_c ?? "" });
    setOpen(true);
  };
  const onChange = (k, v) => setEdit((p) => ({ ...p, [k]: v }));

  const save = async () => {
    if (!edit?._id) return; setSaving(true);
    try {
      const payload = {
        date: edit.date, shift: edit.shift, unit: edit.unit, department: edit.department,
        summary: edit.summary || "", details: edit.details || "", operator: edit.operator || "",
    ...
    } catch (e) { alert(e?.response?.data?.error || e.message || "Failed to save logbook entry"); }
  ...
  const removeOne = async () => {
    ...
    setDeleting(true);
    try { await api.delete(`/api/logbook/${edit._id}`); await load(); setOpen(false); }
    catch (e) { alert(e?.response?.data?.error || e.message || "Failed to delete logbook entry"); }
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="card" style={{ marginTop: 12 }}>
        ...
        <div className="responsive-grid-6" style={{ display: "grid", gridTemplateColumns: "repeat(6, minmax(0,1fr))", gap: 10, marginBottom: 12 }}>
          <input className="input" type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} />
          <select className="select" value={form.shift} onChange={(e) => setForm({ ...form, shift: e.target.value })}><option>A</option><option>B</option><option>C</option></select>
        ...
        <div className="table-responsive">
          <table className="table">
            <thead><tr><th>Date</th><th>Shift</th><th>Unit</th><th>Dept</th><th>Summary</th></tr></thead>
            <tbody>
              {rows.map((r) => (
