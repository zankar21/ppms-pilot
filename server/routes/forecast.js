 ...
 *  - days: history window (default 90)
 *  - alpha: exponential smoothing factor (default 0.35)
 *  - service: service level (default 0.95) → z ≈ 1.65
 *  - lead: default lead time in days if item missing (default 7)
 *  - review: review period days to cover beyond lead time (default 7)
 *
 * Assumptions:
 *  - Transactions in collection "inventory_txns"
 *    fields: { itemId, qty, type, txn_dt }
 *    issue types reduce stock: type in ["ISSUE","CONSUME"]
 *  - Items in collection "inventory_items"
 *    on-hand: one of ["onHand","stock","qty","quantity"] (first it finds)
 *    lead time: "leadTimeDays" if present, else ?lead
 */
router.get("/inventory", async (req, res) => {
  try {
    ...
    // 1) Daily ISSUE/CONSUME totals per item (last N days)
    const issueTypes = ["ISSUE", "CONSUME"];
    const usage = await txnsCol
      .aggregate([
        { $match: { txn_dt: { $gte: since }, type: { $in: issueTypes } } },
        {
          $group: {
    ...
    const results = usage.map((u) => {
      ...
      return {
        ...
        reorderQty,
        runoutDate,
        priority: onHand - rop, // < 0 means below ROP
      };
    });
